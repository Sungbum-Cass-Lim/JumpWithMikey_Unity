...
namespace TournamentSDKUnity
{
     public partial class TournamentUnitySDK : MonoBehaviour
    {
        ...
        public async UniTask<PostResponse> NotifyGameStart(string pid)
        {
            try
            {
                var startResult = await postInvoker.ExcuteAsync<PostResponse>(new NotifyStartGame(pid));
                Debug.Log(String.Format($"single game Start :: {startResult.isSuccess}"));
                return startResult;
            }
            catch (TournamentErrorException ex)
        ...
        public async UniTask<PostResponse> NotifyPVPGameEnd(PVPGameEndData data)
        {
            try
            {
                var endResult = await postInvoker.ExcuteAsync<PostResponse>(new NotifyPVPEndGame(data));
                Debug.Log(String.Format($"single game Start :: {endResult.isSuccess}"));
                return endResult;
            }
            catch (TournamentErrorException ex)
